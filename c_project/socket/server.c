...
int main(){
	...
	//my_addr.sin_addr.s_addr=htonl(INADDR_ANY);/*IP，括号内容表示本机IP*/
	//my_addr.sin_addr.s_addr=inet_addr("10.235.149.235");
	my_addr.sin_addr.s_addr=inet_addr("127.0.0.1");

/*
	int errNo, nOptval= 1 ; 
	if (errNo = setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR,(const void *)&nOptval , sizeof(int)) < 0)  {
		printf("设置socket 为reuse fail %d\n", errNo);
		return -1;
	}
*/
