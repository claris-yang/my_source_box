...
void test_reverse() {
    std::string str = "Hello Beijing!";
    std::reverse(str.begin(), str.end());
    std::cout << str << std::endl;

    char const_str[] = "Hello londan!";
    int len = strlen(const_str);
    std::reverse(&const_str[0], &const_str[len]);

    assert(std::string(const_str) == "!nadnol olleH");
...
void test_map() {
    ...
    std::map<int, int>::iterator it1 = m.find(42);

    std::string str = "nihao";
    unsigned n = str.find("abc");
    if ( n != std::string::npos) {
        std::cout << " can't find string " << std::endl;
...
void test_array() {
    ...
    std::cout << "arr size is " << int_arr.size() << std::endl;
    for (auto n : int_arr) {
        std::cout << n << std::endl;
    }
}
...
template <> void Swap<struct Stu &>(struct Stu &a , struct Stu &b) {
    struct Stu temp;
    temp.age = a.age;
    a.age = b.age;
    b.age = a.age;
...
auto Add(typea a , typeb b) -> decltype(a + b)
{
    return a + b;
}

void song(const char *name = "O . Mr Papa", int times = 1);
...
template <> people & Max<people> (people &a, people &b)
{
    if(a.age > b.age)
    {
        return a;
...
void test_register() {
    ...
    count_fast++;
    std::cout << "register : " << count_fast << std::endl;
}
...
namespace  Jill {
    ...
    {
        std::cout << "Jill :" << n << std::endl;
    }
}

void test_stock() {
    NH::Stock stock("xiaomi");
    stock.show();
}
...
template <typename T>
void fuxi_template(T a) {
    std::cout << a << std::endl;
}


void test_account() {
    Account a("cat", "cat_12345", 10,"beiing");
    Account b("dog", "dog_123456", 30.1,"tianjin");

    std::cout << "zheli" << std::endl;
    Account c = b + a;

    std::cout << "nali" << std::endl;
...
void test_default_account() {
    Account a = 10.1;

}

void test_account_cmp() {
    Account a("cat", "cat_123456", 10.123, "mycat");
    Account b("dog", "dog_123456", 20.123, "mydog");
...
void test_op_double() {
    Account a("tian","acc123", 12.3,"tiananmen");
    a.show_info();
    double del = double(a);
...
void test_account_arr() {
    Account a("tian" , "tian123", 10.21, "tianjin");
...
void test_table() {
    TableTennisPlayer player("jiangfu", "lai");
...
void test_function() {
    ...
    TableTennisPlayer * table =  new RatedPlayer;
    table->ViewAcct();
...
void Swap1(T &a, T &b)
{
    std::cout << "start temp  ok " << std::endl;
    T c;
    c = a;
    a = b;
    b = c;
...
{
    std::cout << "not start temp ok " << std::endl;
    std::string c;
    c = a;
    a = b;
    b = c;
...
void test_longestCommomPrefix() {
    NH::Stock stock;
    std::vector<string> strs = {"nihao","nbeijing","nasbd"};
    std::string result = stock.longestCommomPrefix(strs);
...
void test_addTwoNumbers() {
    NH::Stock stock;
    std::string first = "11", second = "1";
    std::string str = stock.addBinary(first, second);
    std::cout << str << std::endl;
}
...
void *thread2(void *arg) {
    std::cout << "make file buhaoshile " << std::endl;
}
#define MAX_THREAD 100
void test_pthread_function(void *args)
{
    pthread_t thread[MAX_THREAD];
    pthread_t pthread2;

    std::cout << "meiyou kaishi ma " << std::endl;
    if( 0!=pthread_create(&thread[0], NULL, thread1, NULL))
        std::cout << "create ptreahd fail" << std::endl;
    else
        std::cout << "create pthread success" << std::endl;

    if (0!=pthread_create(&pthread2, NULL, thread2, NULL))
        std::cout << "create pthread2 fail" << std::endl;
    else
        std::cout << "create pthread2 success" << std::endl;
...
void s_map() {
    ...
    if (it == m.end()) {
        std::cout << "ok not found 10" << std::endl;
    }

    std::map<int, int>::iterator it1 = m.find(11);
...
std::vector<std::vector<int>> threeSum(std::vector<int>& nums) {
    ...
    for(int i = 0; i < nums.size() &&  nums[i] <= 0 ; i++)
    {
        ...
        int begin = i + 1;
        int end = nums.size() - 1;
        //while(i + 1 < nums.size() && nums[i+1]<=0 && nums[i] == nums[i+1]) i++;
        if (i > 0 && nums[i] == nums[i - 1])
            continue;
        while(begin < end) {

            if (nums[i] + nums[begin] + nums[end] == 0) {
                std::vector<int> one = {nums[i], nums[begin], nums[end]};
                result.push_back(one);

                while (begin + 1 < nums.size() && begin + 1 < end && nums[begin] == nums[begin+1]) begin++;
                if (end - 1 > begin && nums[end-1] == nums[end] ) end--;
...
int solve(int i, int j , std::vector<std::vector<int>> &a , int size)
{
    if (d[i][j] >= 0) return d[i][j];

    return d[i][j] = a[i][j] + ( i == size ? 0 : max(solve(i + 1, j, a, size), solve(i + 1, j+1, a,size)));
}
...
int main(){
    ...
    std::cout << minimumTotal((a)) << std::endl;
}